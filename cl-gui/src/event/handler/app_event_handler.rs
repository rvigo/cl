...
impl<'a> AppEventHandler<'a> {
	pub async fn init(
		...
		let mut app_router = Self { app_rx, app_context: context, ui_context, should_quit };
				...
				AppEvent::Run(command_event) => match command_event {
					CommandEvent::Execute => {
						if let Some(command) = self.ui_context.lock().selected_command() {
							self.app_context.lock().commands.set_command_to_be_executed(command.to_owned());
						}
						self.quit();
					}
					CommandEvent::Insert => {
						...
						match c.add_command(command) {
							...
							Err(error) => {
								...
								ui.popup.set_dialog_type(DialogType::GenericError(error.to_string()));
							}
						}
					}
					CommandEvent::Edit => {
					...
					CommandEvent::Copy => {
						...
						if let Some(command) = ui.selected_command() {
							if let Err(error) = self.app_context.lock().copy_to_clipboard(&command.command) {
								ui.popup.set_show_popup(true);
								ui.popup.set_dialog_type(DialogType::GenericError(error.to_string()));
							} else {
