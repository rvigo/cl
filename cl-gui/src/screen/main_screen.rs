...
use crate::{
	...
	widget::{
		statusbar::Help, tabs::Tabs, text_field::FieldType, ClibpoardWidget, Component, DisplayWidget,
		List,
	},
	State,
};
...
fn create_tabs<'a>(namespaces: Vec<Namespace>, selected: usize) -> Tabs<'a> {
	Tabs::new(namespaces)
		...
		.highlight_style(
			Style::default().fg(DEFAULT_HIGHLIGHT_COLOR).add_modifier(Modifier::BOLD | Modifier::ITALIC),
		)
		.divider('|')
}
...
fn render_medium_size(
	...
	let drawable_chunks =
		Layout::default().direction(Direction::Vertical).constraints(drawable_area).split(frame.size());

	let main_chunks =
		Layout::default().direction(Direction::Horizontal).constraints(areas).split(drawable_chunks[0]);
	...
	let app_name = Paragraph::new(Text::styled(
		...
		Style::default().fg(DEFAULT_WIDGET_NAME_COLOR).add_modifier(Modifier::BOLD | Modifier::ITALIC),
	))
	.alignment(Alignment::Center)
	.block(
		Block::default()
			.borders(Borders::TOP | Borders::RIGHT)
			.style(Style::default().bg(DEFAULT_BACKGROUND_COLOR).fg(DEFAULT_TEXT_COLOR))
			.border_type(BorderType::Rounded)
			.padding(Padding::horizontal(2)),
	);

	let right_side =
		Layout::default().direction(Direction::Vertical).constraints(constraints).split(main_chunks[1]);
